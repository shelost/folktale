<script lang="ts">
	...
	const unsubscribeLang = languageStore.subscribe((lang) => {
		...
		if (showTip && currentTip) {
			// Update tip text if language changes
			const tip = animationData.parentTips.find((t) => t.trigger === 'tigerSpeaks');
			if (tip) {
	...
	const unsubscribeStep = currentStep.subscribe((s) => {
		...
		// Show tip when tiger appears (since we combined motherFalls and tigerSpeaks)
		if (s === 'tigerAppears' && !showTip) {
			const tip = animationData.parentTips.find((t) => t.trigger === 'tigerSpeaks');
			if (tip) {
				currentTip = tip.text[language];
				showTip = true;
